...
if __name__ == '__main__':
    s = T.datetime.now()
    print("################# This program is Nearest Neighbors Model ################################")
    ...
    nameOfFile = file[selectTrain].replace('Trainset_','').replace('SeparateEachEpochPerRow','NearestNeighbors').replace(".csv", ".txt")
    ...
    model = KNeighborsClassifier(n_neighbors=5,weights='distance',algorithm='auto',n_jobs=-1)
    ###############  Crossvalidation  ###################
