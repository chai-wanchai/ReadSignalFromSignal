...
if __name__ == '__main__':
    s = T.datetime.now()
    print("################# This program is Random Forest Classifier Model ################################")
    ...
    text_file = open(path + "\\" + file[selectTrain].replace('Trainset_','').replace('SeparateEachEpochPerRow','RandomForestClassifier').replace(".csv", ".txt"), "w")
    ...
    model = RandomForestClassifier(n_estimators=100,criterion='entropy',class_weight='balanced', n_jobs=-1)
    ###############  Crossvalidation  ###################
